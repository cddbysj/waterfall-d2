...
const domImages = Array.from(document.querySelectorAll('.img-lazyload'))

// 按照瀑布流布局
function waterfall(container, boxs) {
	const domContainer = document.querySelector('#container')
	const domBoxs = Array.from(document.querySelectorAll('.box'))
	// 获取每列的宽度
	const colWidth = domBoxs[0].offsetWidth
	// 瀑布流的列数
	const cols = ~~(clientWidth / colWidth)
	// 设置根容器的宽度
	domContainer.style.cssText = `
		width: ${cols * colWidth}px;
		margin: 0 ${(clientWidth - cols * colWidth) / 2}px;
	`
	// 存储瀑布流一行各个列高的数组
	const colsHeight = []
	// 设置每列
	domBoxs.forEach((box, index) => {
		if (index < cols) {
			colsHeight.push(box.offsetHeight)
		} else {
			// 找出一行中的最小列高，以确定绝对定位的垂直偏移量
			let minColsHeight = Math.min(...colsHeight)
			// 找出最小列高所在的索引，以确定绝对定位的水平偏移量
			let minColsHeightIndex = colsHeight.findIndex(h => h === minColsHeight)
			box.style.cssText = `
				position: absolute;
				left: ${colWidth * minColsHeightIndex}px;
				top: ${minColsHeight}px;
			`
			// 更新列高数组
			// 最小列高更新为加上当前图片高度的和
			colsHeight[minColsHeightIndex] += box.offsetHeight
		}
	})
}
waterfall()
